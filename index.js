...
serveIndex.json = function _json (ctx, req, res, files, next, dir, showUp, icons, path) {
  // stat all files
  stat(path, files, function (err, fileList) {
    if (err) return next(err)
    ...
    send(ctx, res, 'application/json', body)
    next()
  })
};
...
serveIndex.plain = function _plain (ctx, req, res, files, next, dir, showUp, icons, path) {
  // stat all files
  stat(path, files, function (err, fileList) {
    if (err) return next(err)
